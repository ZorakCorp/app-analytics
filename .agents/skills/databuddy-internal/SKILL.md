...
- **`@elysiajs/cors` with `origin: true`** sets `Vary: *`, killing CDN caching. `apps/api/src/routes/public/index.ts` uses `onAfterHandle` to set `set.headers.vary = "Origin"`; reuse on other permissive CORS mounts if needed.
- **`apps/links` redirects**: Exclude soft-deleted rows (`deletedAt` null), same as dashboard; only redirect to validated `http:`/`https:` targets (`targetUrl`, `iosUrl`, `androidUrl`, `expiredRedirectUrl`).
- **`validateUrl` (SSRF guard)**: Resolves **both** A and AAAA; reject if **any** resolved address is private/reserved (`packages/shared/src/ssrf-guard.ts`).
...
- **Agent SQL security**: Tenant isolation (`client_id`) is enforced programmatically in `validateAgentSQL` + `requiresTenantFilter` from `@databuddy/db`. Never rely solely on system-prompt instructions for data isolation. Every SQL tool entry point (API, RPC, etc.) must call the shared gate `assertAgentClickHouseSqlPolicy` from `@databuddy/shared/policy/agent-tool-gateway` (which wraps `packages/db/src/clickhouse/sql-validation.ts`).
- **SSRF policy for fetch/proxy paths**: Use `assertSsrfProtectedHttpUrl` from `@databuddy/shared/policy/agent-tool-gateway` (wraps `packages/shared/src/ssrf-guard.ts`) instead of calling `validateUrl` ad hoc from feature code.
- **CI regression denylist**: Root `bun run security:regression-check` scans `apps/` + `packages/` for a small set of blocked patterns (runs in `.github/workflows/ci.yml` before lint).
